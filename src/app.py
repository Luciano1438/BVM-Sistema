...
def abandonar_taller() -> bool:
    ...
    try:
        ...
        if info["rol"] == "dueño":
            st.error("Como dueño del taller, no podés abandonarlo directamente. Debés disolverlo o transferir la propiedad o eliminarlo.")
            return False
...
chevron_right
Lamento la interrupción en la respuesta anterior. Debido al límite de caracteres en la transferencia de datos, el bloque de código se cortó de forma abrupta. 

Aquí tienes el código completo de `app.py` integrado de principio a fin, sin omisiones ni recortes, listo para producción:
